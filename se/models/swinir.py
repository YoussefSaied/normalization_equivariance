...
# -------------------------------------------------------------------------
# Minimal utilities inlined from timm to avoid the dependency.
# -------------------------------------------------------------------------
...
def trunc_normal_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    # Adapted from timm.layers.weight_init
    def norm_cdf(x):
...
class BasicLayer(nn.Module):
    ...
    def flops(self) -> float:
        flops: float = 0.0
        for blk in self.blocks:
            flops += float(blk.flops())  # type: ignore
        if self.downsample is not None:
            flops += float(self.downsample.flops())
        return flops
...
class RSTB(nn.Module):
    ...
    def flops(self) -> float:
        flops: float = 0.0
        flops += float(self.residual_group.flops())
        H, W = self.input_resolution
        flops += H * W * self.dim * self.dim * 9
        flops += float(self.patch_embed.flops())
        flops += float(self.patch_unembed.flops())
...
class SwinIR(nn.Module):
    r"""SwinIR adapted for image denoising (lite defaults)."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 3) reconstruction head (denoising only)
    ...
    def build_model(cls, model_cfg: ModelConfig, in_channels: int = 1):
        ...
        ), "SwinIR currently supports only ordinary mode."
        return cls(in_chans=in_channels)
